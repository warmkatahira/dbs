...
<x-app-layout>
    ...
    <div class="bg-white">
        <form method="POST" action="{{ route('monthly_cost_setting_create.create') }}" id="monthly_cost_setting_create_form">
            ...
            <x-div.select label="経費項目" id="monthly_cost_setting_item_id" :forValue="$monthly_items" forId="monthly_item_id" forText="monthly_item_name" :db="null" required="1" tippy="0" />
            <x-div.input type="text" label="月額経費" id="monthly_cost_setting" :db="null" required="1" tippy="0" />
        </form>
    </div>
